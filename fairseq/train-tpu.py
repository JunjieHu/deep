...
from fairseq import (
    distributed_utils, options, tasks, utils
)
from fairseq import checkpoint_utils_tpu as checkpoint_utils
from fairseq.data import iterators
from fairseq.trainer_tpu import Trainer
from fairseq import progress_bar_tpu as progress_bar
...
def main(args, init_distributed=False):
    ...
    while lr > args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update:
        ...
        reload_dataset = task.has_sharded_data("train")
        # sharded data: get train iterator for next epoch
        epoch_itr = trainer.get_train_iterator(epoch_itr.next_epoch_idx, load_dataset=reload_dataset)
        print(f'| [LOG] Current epoch={epoch_iter.epoch}, reload_dataset={reload_dataset}, loaded data from the next shard for the next epoch={epoch_itr.next_epoch_idx}')
    train_meter.stop()
...
def train(args, trainer, task, epoch_itr):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if (
            ...
            valid_losses = validate(args, trainer, task, epoch_itr, valid_subsets)
            checkpoint_utils.save_checkpoint(args, trainer, epoch_itr, valid_losses[0])
            # print(' * [DEBUG] validation time', time.time() - cur_valid)
...
def main_tpu(args):
    ...
    def train_loop_fn(device, trainer, loader, last_batch_index):
        ...
        for i, samples in enumerate(loader, start=epoch_itr.iterations_in_epoch):
            ...
            # # VALIDATION by --validate-interval-updates. Added by JJ
            # if (
            #     not args.disable_validation 
            #     and args.validate_interval_updates > 0 
    ...
    while keep_training(lr, epoch_itr, trainer):
        ...
        # VALIDATION by --validate-interval
        if (
            not args.disable_validation 
...
def get_args():
    ...
    parser.add_argument('--save_specific_checkpoints', type=str, default='5000,50000,100000,150000,200000,300000,400000,500000')
