...
def _main(args, output_file):
    ...
    print(f'-[JJ] tokenizer={tokenizer}, bpe={bpe}')
    def decode_fn(x):
        if bpe is not None:
            print('[JJ] before bpe decoding', x)
            x = bpe.decode(x)
            print('[JJ] after bpe decoding', x)
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                ...
                if has_target and j == 0:
                    ...
                    if hasattr(scorer, 'add_string'):
                        print('[JJ] using detok_hypo_str')
                        scorer.add_string(target_str, detok_hypo_str)
                    else:
                        print('[JJ] using hypo_str')
