...
def main(args):
    ...
    while lr > args.min_lr and epoch_itr.next_epoch_idx <= max_epoch:
        ...
        logger.info(f' *[JJ] training time for one epoch: {time.time() - train_start_time} s ')
        if should_stop:
            break
        ...
        logger.info(f' *[JJ] Current epoch={epoch_itr.epoch}, loaded data for the next epoch={epoch_itr.next_epoch_idx}')
        logger.info(f' *[JJ] Load data time={time.time() - load_data_time} s')
    train_meter.stop()
...
def train(args, trainer, task, epoch_itr):
    ...
    num_total_updates = len(progress)
