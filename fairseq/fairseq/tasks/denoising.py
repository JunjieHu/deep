...
class DenoisingTask(FairseqTask):
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if dictionary is None:
            raise FileNotFoundError('dictionary file not found at: {}'.format(args.data))
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        # print('[JJ] before StripTokenDataset dataset[0]', len(dataset[0]), dataset[0][:100])
        # print('[JJ] before StripTokenDataset dataset[0]', len(dataset[0]), dataset[1][:100])
        # dataset = StripTokenDataset(dataset, self.dictionary.eos())

        # print('[JJ] before TokenBlockDataset dataset[0]', len(dataset[0]), dataset[0][:100])
        # print('[JJ] before TokenBlockDataset dataset[0]', len(dataset[0]), dataset[1][:100])
        ...
        # print('[JJ] before PrependTokenDataset dataset[0]', len(dataset[0]), dataset[0][:100])
        # print('[JJ] before PrependTokenDataset dataset[0]', len(dataset[0]), dataset[1][:100])
        # prepend beginning-of-sentence token (<s>, equiv. to [CLS] in BERT)
        dataset = PrependTokenDataset(dataset, self.source_dictionary.bos())
        # dataset = AppendTokenDataset(dataset, self.source_dictionary.eos())
        # [JJ]
        # print('[JJ] PrependTokenDataset dataset[0]', len(dataset[0]), dataset[0][:100])
        # print('[JJ] PrependTokenDataset dataset[0]', len(dataset[0]), dataset[1][:100])

        mask_whole_words = get_whole_word_mask(self.args, self.source_dictionary) \
            if self.args.mask_length != 'subword' else None

        # print('[JJ] mask_whole_words', mask_whole_words, self.args.mask_length)
        ...
        # print('[JJ] DenoisingDataset', len(self.datasets[split][0]))
        logger.info(
