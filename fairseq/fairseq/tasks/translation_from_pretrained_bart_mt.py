...
@register_task('translation_from_pretrained_bart_mt')
class TranslationFromPretrainedBARTMTTask(TranslationTask):
    """
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if split == 'train':
            ...
            # Debug
            # for i in range(100):
            #     src_toks = [self.src_dict[xx] for xx in dae_dataset.src[i].tolist()]
            #     tgt_toks = [self.tgt_dict[xx] for xx in dae_dataset.tgt[i].tolist()]
            #     print(f'{i} src={src_toks[:10]} ... {src_toks[-10:]}')
            #     print(f'{i} tgt={tgt_toks[:10]} ... {tgt_toks[-10:]}')
            #     print('==================================\n')
            # exit(0)

            sample_ratios = [1, 1]
            dataset = [id2dataset[idx] for idx in range(2)]
            dae_id = task2id['dae']
            ...
            dataset = ConcatMultitaskDataset(dataset, sample_ratios, separate_collater=True)
