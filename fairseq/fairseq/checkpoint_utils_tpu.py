...
def save_checkpoint(
    ...
    # print('[JJ] all checkpoints', checkpoints)
    if len(checkpoints) > 0:
        ...
        if not args.no_last_checkpoints:
            last_checkpoint_path = os.path.join(args.save_dir, 'checkpoint_last.pt')
            os.popen(f'cp {checkpoints[0]} {last_checkpoint_path}')
            print('| make a copy to checkpoint_last.pt')
        if args.save_specific_checkpoints is not None:
            save_specific_path = os.path.join(args.save_dir, 'save')
            os.makedirs(save_specific_path, exist_ok=True)
            specific_checkpoints = args.save_specific_checkpoints.split(',')
            if str(updates) in specific_checkpoints:
                os.popen(f'cp {checkpoints[0]} {save_specific_path}/')
                print(f'|[JJ] save specific checkpoint {checkpoints[0]} to {save_specific_path}/')
            else:
                print(f'|[JJ] do not arxiv {checkpoints[0]} since it is not in {specific_checkpoints}')
