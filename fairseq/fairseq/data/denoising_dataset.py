...
def collate(
    ...
    # print('[JJ] denoising_dataset.py: before merge', [len(s['source']) for s in samples])
    src_tokens = merge(
        'source', left_pad=left_pad_source,
        pad_to_length=pad_to_length['source'] if pad_to_length is not None else None,
    )
    # print('***[JJ]*** denoising_dataset.py: after merge', src_tokens.shape)
    ...
    # print('[JJ] denoising_dataset.py: collate batch:', ntokens, src_tokens.shape, target.shape)
    batch = {
        'id': id,
        'ntokens': ntokens,
        'net_input': {
...
class DenoisingDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            # print(f'tokens[-1]={tokens[-1]}, self.eos={self.eos}')
            assert tokens[-1] == self.eos, f'tokens[-1]={tokens[-1]}, self.eos={self.eos}'
