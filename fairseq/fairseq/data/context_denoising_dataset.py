...
class ContextDenoisingDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        self.sizes = np.array(src_sizes).astype(np.int32)
    ...
    def __getitem__(self, index):
        ...
        assert (source >= 0).all()
        # Debug [JJ]:
        # print(f' *[JJ] context_denoising_dataset.py: {index} source', source, 'length = ', len(source))
        # print(f' *[JJ] context_denoising_dataset.py: {index} bos', self.vocab.bos(), self.vocab[self.vocab.bos()], 'eos', self.eos, self.vocab[self.eos])
        # print(f' *[JJ] context_denoising_dataset.py: {index} source text', [self.vocab[s] for s in source])
        # print(f' *[JJ] context_denoising_dataset.py: {index} target text', [self.vocab[s] for s in target])
        # print(f' *[JJ] context_denoising_dataset.py: {index} source text', (source[1:-1] >= 1))
    ...
    def collater(self, samples, pad_to_length=None):
        ...
        if self.task_id is not None:
            res['task_id'] = torch.LongTensor([s['task_id'] for s in samples])
        return res
