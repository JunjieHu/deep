...
cpdef list batch_by_size_fast(
    ...
    for i in range(len(indices_view)):
        ...
        if _is_batch_full(len(batch), num_tokens, max_tokens, max_sentences):
            ...
            # print('[JJ] data_utils_fast.pyx, num_tokens', num_tokens, max_tokens, max_sentences, len(batch))
...
@cython.cdivision(True)
cpdef list batch_by_size_sort_by_length_fast(
    np.ndarray[DTYPE_t, ndim=1] indices,
    num_tokens_fn,
    int64_t max_tokens,
    int64_t max_sentences,
    ...
    for i in range(len(indices_view)):
        idx = indices_view[i]
        num_tokens = num_tokens_fn(idx)
        if num_tokens not in len_dict:
            len_dict[num_tokens] = [idx]
        else:
            len_dict[num_tokens].append(idx)


    for length, length_bin in len_dict.items():
        bsz = max_tokens // length
        num_batch = (len(length_bin) + bsz - 1) // bsz
        for i in range(num_batch):
            batch = length_bin[i * bsz: (i + 1) * bsz]
            if len(batch) > 0:
                batches.append(batch)
    return batches
