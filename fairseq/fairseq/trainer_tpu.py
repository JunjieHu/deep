...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        print(' - [JJ] trainer_tpu.load_checkpoint', bexists, filename)
        if bexists:
            ...
            print(' - [JJ] finish loading checkpoint to cpu')

        if last_optim_state is not None and not reset_optimizer:
            # rebuild optimizer after loading model, since params may have changed
            print(' - [JJ] restore optimizer')
            self._build_optimizer()
    ...
    def get_train_iterator(self, epoch, combine=True, load_dataset=True, data_selector=None):
        ...
        if load_dataset:
            ...
            # [JJ]
            # print('[JJ] before calling self.task.get_batch_iterator()')
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        # print(' * in train step, |samples|=', len(samples))
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    # [JJ]
                    # print(' * [JJ] trainer_tpu: before self.task.train_step', len(sample), sample.keys())
                    ...
                    # print(' * [JJ] trainer_tpu: get the loss in trainer_tpu.py', loss)
                    # print(' * [JJ] forward time:', time.time() - forward_time)
            ...
            # print(' * [JJ] updating each sample time', time.time() - sample_start_time)
    ...
    def meters_to_device(self, device):
        ...
        """
        for mkey, meter in self.meters.items():
