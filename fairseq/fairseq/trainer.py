...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, quantizer=None):
        ...
        # [JJ]: multitask
        self.multitask = getattr(args, 'multitask', None)
        if self.multitask:
            self.multitask = self.args.tasks.split(',')
            self._multitask_num_batches = [0] * (len(self.multitask) + 1)
            self._multitask_primary_task_id = getattr(args, 'primary_task_id', 0)
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            # print(' - [JJ] trainer.py: finish load_checkpoint_to_cpu')
    ...
    def get_train_iterator(
        ...
        if load_dataset:
            ...
            logger.info(f" [JJ] load data by self.task.load_dataset for epoch={epoch}")
        return self.task.get_batch_iterator(
            ...
            epoch=epoch,
            sort_by_length=getattr(self.args, 'sort_by_length', None),
        )
    ...
    def compute_grad_norm(self):
        ...
        for n, p in self.model.named_parameters():
            if p.grad is not None:
                ...
                print(f'-- {n}\t{norm}\t{torch.isfinite(p.grad).all()}')

        print(f'--total norm {total_norm}')

        # params = list(self.optimizer.params)
        ...
        #     )
        # print('[JJ]fairseq.trainer.py: total norm', total_norm)
    ...
    def compute_param_norm(self):
        ...
        for n, p in self.model.named_parameters():
            norm = p.data.norm(2)
            total_norm += norm.item() ** 2
            print(f'-- {n}\t{norm}\t{torch.isfinite(p.data).all()}')
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            # Debug [JJ]
            #_temp_shape = sample['net_input']['src_tokens'].shape
            #_temp_tokens = _temp_shape[0] * _temp_shape[1]
            #ttag = self.task.src_dict[sample['net_input']['src_tokens'][0].tolist()[0]]
            # src0_toks = [self.task.src_dict[xx] for xx in sample['net_input']['src_tokens'][0].tolist()]
            # tgt0_toks = [self.task.tgt_dict[xx] for xx in sample['target'][0].tolist()]
            # print(f'[JJ] before sample: source', src0_toks)
            # print(f'[JJ] before sample: target', tgt0_toks)
            # if ttag == '[dae]' or ttag == '<pad>':
            #     dae_tokens += _temp_tokens
           # elif ttag == '[mt]':
          #      mt_tokens += _temp_tokens
            # elif ttag == '<pad>':
            #     print(f' -pad seq: src={src0_toks}\n       tgt={tgt0_toks}')

            # print(f'  {i}/{len(samples)} -shape', _temp_shape, ttag, f'dae_tokens={dae_tokens}, mt_tokens={mt_tokens}')
            
            # _temp_batch += _temp_shape
            # print(f"\n  =[JJ] trainer.py: idx={idx}/{len(samples)} sample shape={sample['net_input']['src_tokens'].shape}")
            # #, batch tokens={_temp_shape}, cum_batch tokens={_temp_batch}\n") #, sample['net_input']['src_tokens'])
            # print('[JJ] trainer.py: sample task_id', sample['task_id'])
            # for jj in range(1):
            #     src0_toks = [self.task.src_dict[xx] for xx in sample['net_input']['src_tokens'][jj].tolist()]
            #     src0 = ''.join(src0_toks)
            #     tgt0_toks = [self.task.tgt_dict[xx] for xx in sample['target'][jj].tolist()]
            #     tgt0 = ''.join(self.task.tgt_dict[xx] for xx in sample['target'][jj].tolist())
            #     print('size: src_tokens=', sample['net_input']['src_tokens'].shape, sample['target'].shape)
            #     print(f'src[{jj}]=', [(ss,tt) for (tt,ss) in enumerate(src0_toks)])
            #     print(f'tgt[{jj}]=', [(ss,tt) for (tt,ss) in enumerate(tgt0_toks)])
            #     # print(f'src[{jj}]=', src0.replace('▁', ' '))
            #     # print(f'\ntgt[{jj}]=', tgt0.replace('▁', ' '))
            #     # mask0 = sample['mask_index'][jj].tolist()
            #     # print(f'mask[{jj}]=', mask0)
            #     # print('masked entities=', [src0_toks[xx] for xx in mask0])
            #     print('\n###########################')
            # print('\ncontext=', ''.join(self.task.dictionary[xx] for xx in sample['context'][0].tolist()).replace('▁', ' '))
            # if idx % 65 == 0:
            #     exit(0)
            ...
            if sample is None:
                ...
                print('[JJ] prepare dummy batch at ', i)
            ...
            if 'net_input' not in sample:
                print(f'[JJ] keyError at {i} / {len(samples)}', sample, is_dummy_batch)
                continue
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                self.optimizer.step()
                # print(f'[JJ] self.optimizer.step() in iter {idx}')
                # print('=====after step ======')
                # self.compute_param_norm()
                # exit(0)
        ...
        if not overflow or self.args.distributed_wrapper == 'SlowMo':
            # [JJ]: for multitask
            # if self.multitask:
            #     self.update_multitask_num_batches(samples)
            #     self.set_num_updates(self.get_multitask_num_updates(self._multitask_primary_task_id) + 1)
            #     # print(f'[JJ] trainer.py: self._multitask_num_batches', self._multitask_num_batches, [s['task_id'] for s in samples])
            #     # print(f'[JJ] trainer.py: self.get_multitask_num_updates', self.get_multitask_num_updates(self._multitask_primary_task_id), self.get_num_updates())
            # else:
    ...
    # def get_multitask_num_updates(self, task_id):
    #     """Get the number of parameters updates for the primary task."""
    #     return self._multitask_num_batches[task_id] // self.args.update_freq[0]
    ...
    # def update_multitask_num_batches(self, samples):
    #     for sample in samples:
    #         for task_id in sample['task_id'].tolist():
    #             self._multitask_num_batches[task_id] += 1
